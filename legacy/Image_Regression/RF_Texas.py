...
t1_test = np.array(mat['A'],dtype = float)
t2_test = np.array(mat['B'], dtype = float)
idx = np.where(mask)[0]
...
regr = rf(n_estimators = trees ,max_features="sqrt",oob_score=True,n_jobs=-1,min_samples_leaf = m)
...
scipy.io.savemat('RF_Texas_super_test',{'d_RF': d})
