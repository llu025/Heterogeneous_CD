...
#Read data
mat = scipy.io.loadmat('Texas_super_test.mat')
mask = np.array(mat['mask'],dtype = bool)
t1 = np.array(mat['A'],dtype = float)
t2 = np.array(mat['B'], dtype = float)
idx = np.where(mask)[0]
...
for i in range(1+t1.shape[0]/31000):
    ...
    print "************"
    print "Elapsed time: ", et//3600, " hours, ", (et%3600)//60, " minutes, ", (et%3600)%60, " seconds"
    print "Average time. ", ((et/(i+1))%3600)//60, " minutes, ", ((et/(i+1))%3600)%60, " seconds"
    print "Time to go: ", remaining//3600, " hours, ", (remaining%3600)//60, " minutes, ", (remaining%3600)%60, " seconds"
    print "************"
del regr1, temp1
...
d2 = d2/np.nanmax(d2)
d = d1 + d2
scipy.io.savemat('GPR_Texas_super_test',{'d_GPR': d})
